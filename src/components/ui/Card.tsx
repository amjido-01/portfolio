...
export const Card: React.FunctionComponent<CardProps> = ({card}) => {
  ...
  return (
    <main
      key={card.id}
      className={` ${isFirstCard ? 'bg-[#13338c] w-[12rem]' : "bg-transparent border-2  w-[23.9375rem] h-[17.47263rem] md:h-[25rem] md:w-[34.25rem] "} relative rounded-[1.5rem] overflow-hidden
  `}>
      {isFirstCard ? (
        <div className="absolute flex items-center justify-center w-full h-full md:h-full md:w-full">
          <div className="">
